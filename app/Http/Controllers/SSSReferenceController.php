...
use App\Imports\SSSTableImport;
use Maatwebsite\Excel\Facades\Excel;
use Maatwebsite\Excel\HeadingRowImport;
use Illuminate\Support\Str;

class SSSReferenceController extends Controller
{
    /**
    ...
    {
        //
        $data = SSSReference::Orderby('id','ASC')->get();
        return view('admin.ssstable.index', compact('data'));
    }
    ...
    public function import(Request $request)
    {
        ...
        if($extension != 'xls')
            return back()->with('error', 'Data Imported Failed!\n'.'Invalid SSS Table-invalid extension. Please check or download the template first');    
        ...
        if($rows[0][0] == null || Str::Upper($rows[0][0][0]) != Str::Upper('COMPENSATION FROM'))
            return back()->with('error', 'Data Imported Failed!\n'.'Invalid SSS Table File. Please check or download the template first');           
        ...
        SSSReference::truncate();

        $import = new SSSTableImport();
